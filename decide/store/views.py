...
class StoreBotView(APIView):
    ...
    def post(self, request):
        ...
        voting= Voting.objects.get(id=request.data.get('voting_id'))
        ...
        if not vid or not uid or not a or not b:
            return Response({'Te falta algo'}, status=status.HTTP_400_BAD_REQUEST)

        # the user is in the census
        perms = mods.get('census/{}'.format(vid), params={'voter_id': uid}, response=True)
        if perms.status_code == 401:
            return Response({'Censuuusss'}, status=status.HTTP_401_UNAUTHORIZED)
